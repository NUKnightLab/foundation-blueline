...
CONFIG = {
    ...
    'deploy_bucket_name': 'purpleline.knightlab.com'
}
...
env.foundation_scss_path = join(env.project_path, 'source', 'bower_components', 'foundation', 'scss')
env.purpleline_scss_path = join(env.project_path, 'source', 'scss')
env.foundation_js_path = join(env.project_path, 'source', 'bower_components', 'foundation', 'js')
env.purpleline_js_path = join(env.project_path, 'source', 'js')
...
def _pyScss(src, dst, options=''):
    """Compile SCSS"""
    if not os.popen('which pyScss').read().strip():
        abort('You must install the pyScss compiler')

    _makedirs(dst, True)

    with hide('warnings'), settings(warn_only=True):
        result = local('pyScss %s %s > %s' % (options, src, dst))
    if result.failed:
        abort('Error running pyScss on %s' % src)
...
def build():
    ...
    # copy
    _copy(join(env.project_path, 'source'),
          env.build_path,
          re.compile(r'(css/.*|font/.*|img/.*|.*\.html)$'))

    _copy(join(env.foundation_js_path, 'foundation'),
            join(env.build_path, 'js'))

    # compile uncompressed css
    _pyScss(
        join(env.project_path, 'source', 'scss', 'purpleline.scss'),
        join(env.build_path, 'css', 'purpleline.css'),
        '--load-path %s --load-path %s --no-compress' % (env.foundation_scss_path, env.purpleline_scss_path))

    # compile compressed css
    _pyScss(
        join(env.project_path, 'source', 'scss', 'purpleline.scss'),
        join(env.build_path, 'css', 'purpleline.min.css'),
        '--load-path %s --load-path %s' % (env.foundation_scss_path, env.purpleline_scss_path))

    # concatenate files
    # TODO: Add one of these that concats the SCSS files to make one hyooj purpleline-plus.css file
    _concat(
        map(lambda x: join(env.foundation_js_path, 'foundation', x), [
            'foundation.abide.js',
            ...
            'foundation.js'
        ]), join(env.build_path, 'js', 'purpleline-plus.js'))

    _concat(
        map(lambda x: join(env.foundation_js_path, 'foundation', x), [
            # Add other PurpleLine-specific files not from Foundation here!
            'foundation.js'
        ]), join(env.build_path, 'js', 'purpleline.js'))

    # TODO: Copy each Foundation module in its minified and complete forms to the build JS directory

    # minify
    _minify(join(env.build_path, 'js', 'purpleline-plus.js'))
    _minify(join(env.build_path, 'js', 'purpleline.js'))
    ...
    _banner([
        join(env.build_path, 'js', 'purpleline.js'),
        join(env.build_path, 'js', 'purpleline.min.js'),
        join(env.build_path, 'css', 'purpleline.css'),
        join(env.build_path, 'css', 'purpleline.min.css')
    ])
...
def stage():
    ...
    _copy(env.build_path, cdn_path,
        re.compile(r'(img/.*|assets/.*|css/.*|font/.*|js/.*)$'))
